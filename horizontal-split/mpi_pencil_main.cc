...
int main(int argc, char *argv[])
{
    ...
    SUCCESS_OR_DIE(MPI_Cart_shift(/*MPI_Comm comm: */ comm,
                                  /*int direction: */ 0,
                                  /*int disp: */ 1,
                                  /*int *rank_source: */ &neighbours_ranks[DIRECTIONS::UP],
                                  /*int *rank_dest: */ &neighbours_ranks[DIRECTIONS::DOWN]));
    // Get my rank in the new communicator
    SUCCESS_OR_DIE(MPI_Comm_rank(comm, &my_rank));

    /* HWLOC TOPO SETUP START */
...
//               << "has pid: " << getpid() << " "
//               << "coreid: " << hwloc_bitmap_first(set) << std::endl;
//     /* ---------------------- DEBUG ---------------------- */
// #endif

    /* HWLOC TOPO SETUP END */
...
#pragma message("OMP_NUM_THREADS")
    ...
    // std::atoi(std::getenv("SLURM_CPUS_PER_TASK"))
...
#define ENERGY_INIT args[PARAMS_ARGS::ARG_ENERGY_INIT]
    ...
    const size_t nbr_of_row_local = (nbr_of_row + comm_size - 1) / comm_size; // block size in x
#define MPI_coords_x coords[0]
#define offset_row (MPI_coords_x * nbr_of_row_local) // offset in x
    MPI_Win windows[2];
    double *windows_buffer[2];
    const MPI_Aint segment_size =
        (/*Local segment size*/ (nbr_of_col * nbr_of_row_local) +
         /*Add TOP and BOTTOM halo row exchange*/ (2 * nbr_of_col));
#if defined(PRINT_PERF)
...
/* ---------------------- END DEBUG ---------------------- */
    ...
    for (size_t nb = 0; nb < NBR_ITERS; nb++)
    {
        ...
        SUCCESS_OR_DIE(MPI_Rget(
            /* void *origin_addr = */ &(windows_buffer[0][1]),
            /* int origin_count = */ 1,
            /* MPI_Datatype origin_datatype = */ north_south_type,
            /* int target_rank = */ neighbours_ranks[DIRECTIONS::UP],
            /* MPI_Aint target_disp = */ offset_array_north + 1,
            /* int target_count = */ 1,
            /* MPI_Datatype target_datatype = */ north_south_type,
            /* MPI_Win win = */ *windows,
            /* MPI_Request *request = */ &reqs[0]));
        SUCCESS_OR_DIE(MPI_Rget(
            /* void *origin_addr = */ &(windows_buffer[0][offset_array_south]),
            /* int origin_count = */ 1,
            /* MPI_Datatype origin_datatype = */ north_south_type,
            /* int target_rank = */ neighbours_ranks[DIRECTIONS::DOWN],
            /* MPI_Aint target_disp = */ nbr_of_col + 1,
            /* int target_count = */ 1,
            /* MPI_Datatype target_datatype = */ north_south_type,
            /* MPI_Win win = */ *windows,
        ...
        // /* IN_PART */
        // compute_vectorized_I(/* double in_host = */ windows_buffer[0],
        //                      /* double out_host = */ windows_buffer[1],
        //                      nbr_of_col, nbr_of_row_local,
        //                      col_block_size, row_block_size,
        //                      col_block, row_block,
        //                      /* std::initializer_list<size_t> col_idx_slice_idx = */
        //                      {col_block_size, col_block_size * (col_block - 1)},
        //                      /* std::initializer_list<size_t> row_idx_slice_idx = */
        //                      {row_block_size, row_block_size * (row_block - 1)});

        // /* EAST_PART */
        // compute_vectorized_E(/* double in_host = */ windows_buffer[0],
        //                      /* double out_host = */ windows_buffer[1],
        //                      nbr_of_col, nbr_of_row_local,
        //                      col_block_size, row_block_size,
        //                      col_block, row_block,
        ...
        // /* WEST_PART */
        // compute_vectorized_W(/* double in_host = */ windows_buffer[0],
        //                      /* double out_host = */ windows_buffer[1],
        //                      nbr_of_col, nbr_of_row_local,
        //                      col_block_size, row_block_size,
        //                      col_block, row_block,
        ...
        /* MPI Recovery must be done */

        // /* NORTH_PART */
        // compute_vectorized_N(/* double in_host = */ windows_buffer[0],
        //                      /* double out_host = */ windows_buffer[1],
        //                      nbr_of_col, nbr_of_row_local,
        //                      col_block_size, row_block_size,
        //                      col_block, row_block,
        ...
        //                      /* row_idx_slice_idx std::initializer_list<size_t> = */
        //                      {0, row_block_size});

        // /* SOUTH_PART */
        // compute_vectorized_S(/* double in_host = */ windows_buffer[0],
        //                      /* double out_host = */ windows_buffer[1],
        //                      nbr_of_col, nbr_of_row_local,
        //                      col_block_size, row_block_size,
        //                      col_block, row_block,
        ...
        //                      {0, col_block_size * col_block},
        //                      /* std::initializer_list<size_t> row_idx_slice_idx = */
        //                      {row_block_size * (row_block - 1), row_block_size * row_block});
